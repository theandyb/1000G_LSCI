...
get_singleton_pos <- function(subtype, rp, sp){
  nucs <- c("A", "C", "G", "T")

  pos <- rp + 11

  singleton_dir <- "/net/snowwhite/home/beckandy/research/1000G_LSCI/output/singletons/"
  fName <- paste0(singleton_dir, sp, "/", subtype, ".txt")
...
get_control_pos <- function(subtype, rp, sp){

  nucs <- c("A", "C", "G", "T")
  pos <- rp + 11

  control_dir <- "/net/snowwhite/home/beckandy/research/1000G_LSCI/output/controls/"
  fName <- paste0(control_dir, sp, "/", subtype, ".txt")
...
#' Get genome-wide counts of nucleotides flanking A or C (6 category version)
#'
...
get_gw_position <- function(nucleotide, rp){
  ...
  data_dir <- "/net/snowwhite/home/beckandy/research/1000G_LSCI/output/gw_1_count/6cat/"
...
get_all_position <- function(subtype, rp, sp){
  if(str_starts(subtype, "A")){
    nuc <- "A"
  } else if(str_starts(subtype, "C")){
    nuc <- "C"
  } else if(str_starts(subtype, "G")) {
    nuc <- "G"
  } else if(str_starts(subtype, "T")) {
    nuc <- "T"
  } else if(str_starts(subtype, "cpg_G")) {
    nuc <- "cpg_G"
  } else if(str_starts(subtype, "cpg_C")) {
    nuc <- "cpg_C"
  }

  df_s <- get_singleton_pos(subtype, rp, sp)
  df_c <- get_control_pos(subtype, rp, sp)
...
subtypes <- c("A_C", "A_G", "A_T",
              "C_A", "C_G", "C_T",
              "G_A", "G_C", "G_T",
              "T_A", "T_C", "T_G",
              "cpg_C_A", "cpg_C_G", "cpg_C_T",
              "cpg_G_A", "cpg_G_C", "cpg_G_T")
...
for(sp in c("ALL", "AFR", "AMR", "EAS", "EUR", "SAS")){
  ...
  for(st in subtypes){
    ...
    results[[st]] <- foreach(x = c(-10:-1,1:10), .combine = 'rbind') %dopar% {
      get_all_position(st, x, sp)
    }
  }
  stopImplicitCluster()

  out_dir <- paste0("/net/snowwhite/home/beckandy/research/1000G_LSCI/output/single_pos_df/", sp, "/6cat/")
  for(st in subtypes){
    message(st)
    df <- results[[st]]
